...
{
    public class GoaledSequencer<T> : ISequencer<T>
    {
        ...
        private void AdvanceByLastTrainingDistance()
        {
            ...
            _count = 0;
            _advanceAction = AdvanceByLastRestituteDistance;
            _advanceAction();
        }

        private void AdvanceByLastRestituteDistance()
        {
            _count += 1;
            if (_count <= _repeatTrainingDistance)
            {
                _current = _restituteDistance;
                return;
            }
            if (_count == _repeatTrainingDistance + 1)
            {
